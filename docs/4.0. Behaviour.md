...
In order to trigger an activation a client POSTs an `activation` object in the `activations` endpoint. The `mode` property MAY be set to any one of the following four values:
