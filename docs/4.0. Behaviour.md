...
* It is routed back to the Input of the Audio Channel Mapping API (see [Re-entrant Mappings](#re-entrant-mappings)).
...
In some cases Inputs may not be capable of performing a re-ordering operation with their channels. In such cases the Audio Channel Mapping API may be considered as a means of routing multiple channels of audio within a Device, rather than re-ordering channels, as in the following example:
...
Once an activation has been completed the resulting changes to the map MUST be reflected in the `active` endpoint. The `activation` resource in the `active` endpoint MUST contain the details of the last activation to have taken place.
...
If an Output is modified by an already scheduled activation the API SHOULD respond with HTTP 423 `resource locked`. The API MUST reject the entire request, even if some of the routes in the failed request are not currently locked by another activation.
...
All timestamps in this API MUST be string formatted timestamps (\<seconds\>:\<nanoseconds\>) indicating time (absolute or relative) for activation. For relative times, the time '0:0' MUST be considered to be the time the request is received by the API.

In order to ensure consistent operation of the API during and around leap seconds the API MUST use TAI for all absolute timestamps. TAI does not apply leap seconds, and is therefore an integer number of seconds different to UTC. The timestamps used in the API MUST be based on the UNIX epoch, meaning 1970-01-01 00:00:00 TAI would be represented as '0:0'.
...
In this scenario the audio undergoes two distinct routing operations - the first from the MADI input to the MADI card, and then a second from the MADI card to the AES67 output. In order to allow for these kinds of restrictions the Audio Channel Mapping API supports re-entrant mappings, in which audio at an Output may be routed back to an Input. This is signaled to the control system by the same Source ID being present in both the Output's `sourceid` resource and also the Input's `parent` resource.
