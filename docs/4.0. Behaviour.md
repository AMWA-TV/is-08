...
This specification does not define the channel mapping behaviour at start-up, as this may depend on the nature of the Device the API is controlling. However, the channel mapping behaviour of the underlying Device MUST be reflected by the API at start-up.
...
All Input and Output identifiers MUST be immutable. Input identifiers must be unique within the set of all Input identifiers used in the API [alternatively, "no two Inputs may have the same identifier"], and Output identifiers must be unique in the set of all Output identifiers used in the API. An Output MAY share an identifier with an Input.
...
* is or may be passed to another process within the Device that would not constitute the creation of a new Source or Flow, but would or may then route it's resultant output to an NMOS Sender, OR
...
In the case where an Input is not currently receiving any audio (because, for instance, the Sender of the audio has been disabled or the network has been partitioned), but would pass any audio received in the future onto its Output, then the `parent` resource SHOULD populated the Receiver or Source ID it is associated with as it would if receiving audio. For example, if an NMOS Receiver is not currently receiving a stream, but would pass any audio it received in future onto an Audio Channel Mapping API Input then that input's `parent` field should be populated as if it were currently receiving audio from that Receiver.

### Channels

Inputs and Outputs MUST have at least one channel listed in its `channels` resource. For some Devices the number of channels will be fixed - for example where an Input or Output represents a base-band connection to a Device such as analog audio or AES.
...
A change to the number of channels in an Input or Output MUST be represented in the `channels` resource of that Input or Output. It also MUST be represented in the global `map/active` and `map/staged` resources, as well as in the global `io` resource.
...
A request to create a mapping that would violate these constraint SHOULD cause the API to return an HTTP 400 response. This response SHOULD contain a message informing the client that the request was rejected because the it did not respect an input constraint and also provide information about which constraint was broken, and for which channels/Inputs, in order to aid debugging.

#### Re-ordering

In some cases Inputs may not capable of performing a re-ording operation with their channels. In this case the Audio channel mapping API may be thought of as being used more as a way of routing multiple channels of audio within a Device, rather than re-ordering them. This is illustrated in the diagram below, where channels from a 64 channel MADI input may be mapped into an 8 channel MADI card:
...
* the difference between Input and Output channel indexes MUST be numerically equal for all channels in the mapping between a particular Input and Output, and
...
Not all Devices allow the routing of channels in any Input to any Output. In order to reflect this limitation, Each Output presents a list of Inputs that may be routed to it in the `routableinputs` resource. These MUST be populated by the Device, such that if an Input is listed this list then channels from that input can be routed to that Output in the Map table.
...
Where an Output does not have a channel routed to it, it MUST interpret the absence of the channel in one of the following ways:  [or are other interpretations permissible?]
