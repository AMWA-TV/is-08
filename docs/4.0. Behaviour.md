...
Some Devices place a further constraint that channels must be treated as a discrete block from a given Input source, where all channels in the block are expected to be routed together from a certain starting point, such that:
...
* All blocks MUST start with an input channel index being a multiple of the Offset parameter or 0.

Where an Input is constrained in this way the `offset_step` parameter in the Input's `caps` resource SHALL be an integer indicating the block size that is in use for that Input and the resulting Input Channel start. Otherwise the `offset_step` parameter SHALL be set to `null`.

The following example describes a case, where the specific arcitecture of a given MADI Card allows only to either process Inputs 1-8, 9-16, 17-24, and so on from a given MADI Signal. The following output would be permissible, where an `offset_step` value of `8` is used:
...
The `offset_step` and `reordering` parameters may both be used at the same time, to express a constraint where channels must be treated as blocks, and re-ordering in those blocks is not permitted. For example, the following would no longer be permitted when using an `offset_step` block size of `8` when the `reordering` parameter is set to `false`:
