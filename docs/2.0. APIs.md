...
The Audio Channel Mapping API is specified using:
...
JSON schemas are included with the RAML API definitions. These include validation for values used within the APIs. It is strongly recommended that implementers of a Audio Channel Mapping API use these JSON schemas as part of a validation stage when receiving interactions from clients. Invalid payloads should cause a 400 HTTP error (Bad Request) to be returned to the client.
...
Nodes MAY extend the path for the Audio Channel Mapping API with a suffix. A Node implementation might choose to extend the path so that Devices on the Node have separate Audio Channel Map APIs without having to use a different port for each Device's API. Such a path SHALL be formatted as

```
http(s)://<ip address or hostname>:<port>/x-nmos/<api type>/<api version>/<device selector>/
```
...
This permits succinct local identifiers such as `slot2B`. Equally, using a UUID, such as the associated IS-04 Device `id`, would also be valid.
