...
The following shows an example of an IS-08 Map between one Input and two Outputs, represented as a logical wiring diagram and as a routing matrix:

![terminology example](images/terminology.png "Terminology example")

Note that:

- The eight Output A channels are routed as a block to the first eight Input channels. This could be due to hardware limitationson those channels, which can be signalled as a constraint in IS-08 (see below).
- Channels 7 and 8 of each Output are routed to the same Input channels.
- Channels 5 and 6 of Output B are parked.


The above Map is represented in JSON in the API as follows:
